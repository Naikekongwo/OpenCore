...
PackageManager::PackageManager(string_view pName, ResourceInfo resInfo)
  ...
  auto packageOCData = fs::path(reinterpret_cast<const char8_t *>(
      (string("data//") + string(packageName) + string("_00.ocdata")).c_str()));
  ...
  if (!resourceInfo.packageOnly) {
    ...
    throw runtime_error("packageOnly mode enabled but .ocdata file not found.");
  }
}
...
bool PackageManager::registerResource(ResourceNode resource) {
  ...
  if (!manifest.good()) {
    LOG("发生了意外的错误，该清单文件不存在 {}", packageManifestFile.string());
    return false;
  }
...
bool PackageManager::registerResources(
    initializer_list<ResourceNode> resources) {
  bool result = true;
...
bool PackageManager::generatePackage(const fs::path &manifestPath,
                                     bool cleanup) {
  // 读取原始清单
...
// ──────────────────────────────────────────────
std::vector<char>
PackageManager::extractResourceData(const ResourceNode &node) {
  string ocdataPath = string("data//") + packageName + string("_00.ocdata");
  fstream file(fs::path(reinterpret_cast<const char8_t *>(ocdataPath.c_str())),
               ios::in | ios::binary);
  if (!file.is_open()) {
...
std::shared_future<void> PackageManager::requestTextureLoad(string_view name) {
  ...
  ThreadManager::getInstance().submit(
      [this, name = string(name), promise = std::move(promise)] {
        ...
        if (!surface) {
          LOG("IMG_Load_IO 加载纹理 \"{}\" 失败: {}", name, SDL_GetError());
          promise->set_value();
          return;
        }
...
std::shared_future<void> PackageManager::requestFontLoad(string_view name,
  ...
  ThreadManager::getInstance().submit([this, name = string(name), key, ptsize,
    ...
    if (!font) {
      LOG("TTF_OpenFont_IO 加载字体 \"{}\" 失败: {}", name, SDL_GetError());
      promise->set_value();
      return;
    }

    // TTF_Font 可能引用 IO 流的内存，data 必须和字体同寿命
    auto dataKeepAlive = std::make_shared<std::vector<char>>(std::move(data));
    auto fontPtr = shared_ptr<TTF_Font>(font, [dataKeepAlive](TTF_Font *f) {
      (void)dataKeepAlive;
      TextureDeleter{}(f);
    });
