...
void PackageManager::evictStaleEntries()
{
    ...
    //    遍历删除 use_count 为 1 的纹理/字体，即只有缓存自身持有的
    {
        ...
        // ③ 通用二进制资源（Asset/Audio）的自动垃圾回收
        for (auto it = dataCache_.begin(); it != dataCache_.end();)
        {
            if (it->second.use_count() == 1) // 仅缓存持有
            {
                LOG("淘汰二进制资源缓存: {}", it->first);
                it = dataCache_.erase(it);
            }
            else
                ++it;
        }
    ...
    if (manifest.good())
    {
        ...
        while (getline(manifest, line))
        {
            if (!line.empty())
            {
                ...
                if (matches(node))
                {
                    // 从文件加载到内存缓冲（保留正确的 startIndex/endIndex）
                    if (timer)
                        node.expireTime = timer->getTotalTime() + 2 * EVICT_TTL;
...
// ──────────────────────────────────────────────
//  getData — 通用二进制资源两阶段获取（Asset/Audio 共用核心）
// ──────────────────────────────────────────────
shared_ptr<vector<char>>
PackageManager::getData(string_view name, ResourceType rType, bool blocking)
{
    string key(name);
    ...
    // 阶段 2：已有异步加载在进行中
    {
        ...
        if (it != pendingData_.end())
        {
            ...
            return cached != dataCache_.end() ? cached->second : nullptr;
        }
    }

    if (!blocking)
    {
        // 阶段 3a：非阻塞，触发异步加载后立即返回
        requestDataLoad(name, rType);
        return nullptr;
    }
    ...
    auto bytes = std::make_shared<vector<char>>(std::move(data));
    {
        std::lock_guard<std::mutex> lock(cacheMutex_);
        dataCache_[key] = bytes;
    }
    return bytes;
}

// ──────────────────────────────────────────────
//  requestDataLoad — 异步加载二进制资源
// ──────────────────────────────────────────────
std::shared_future<void> PackageManager::requestDataLoad(string_view  name,
                                                         ResourceType rType)
{
    auto promise = std::make_shared<std::promise<void>>();
    auto fut     = promise->get_future().share();
    ...
    {
        std::lock_guard<std::mutex> lock(cacheMutex_);
        pendingData_[string(name)] = fut;
    }

    // 在后台线程提取原始字节，不涉及渲染器，线程安全
    ThreadManager::getInstance().submit(
        [this, name = string(name), rType, promise = std::move(promise)]
        {
            ResourceNode *node = findNode(name);
            if (!node || node->rType != rType)
            {
                LOG("资源 \"{}\" 未注册或类型不匹配", name);
                promise->set_value();
                return;
            }
            ...
            {
                std::lock_guard<std::mutex> lock(cacheMutex_);
                dataCache_[name] = std::move(bytes);
                pendingData_.erase(name);
            }
            promise->set_value();
        });
...
// ──────────────────────────────────────────────
//  getAsset / getAudio — 通用二进制与音频资源的公开入口
// ──────────────────────────────────────────────
shared_ptr<vector<char>> PackageManager::getAsset(string_view name)
{
    return getData(name, RscAsset, false);
}

shared_ptr<vector<char>> PackageManager::getAssetAsync(string_view name)
{
    return getData(name, RscAsset, true);
}

shared_ptr<vector<char>> PackageManager::getAudio(string_view name)
{
    return getData(name, RscAudio, false);
}

shared_ptr<vector<char>> PackageManager::getAudioAsync(string_view name)
{
    return getData(name, RscAudio, true);
}
